...
public class Cleaning_Service extends AbstractItem {

    private UUID playerUUID;

    public Cleaning_Service(UUID playerUUID) {
        this.playerUUID = playerUUID;
    }
    @Override
    public ItemProvider getItemProvider() {
        if ((boolean) accessPlayerData(playerUUID, "Cleaning_Service")) {
            return new ItemBuilder(Material.WHITE_STAINED_GLASS_PANE)
                    .setDisplayName("§eCleaning Service §61☽")
                    .addLoreLines("§7Dear more damage to players under §c5❤")
    ...
    public void handleClick(@NotNull ClickType clickType, @NotNull Player player, @NotNull InventoryClickEvent inventoryClickEvent) {
        ...
        if (clickType.isLeftClick()) {
            if (SP >= 1) {
                storePlayerData(playerUUID, "Cleaning_Service", true);
