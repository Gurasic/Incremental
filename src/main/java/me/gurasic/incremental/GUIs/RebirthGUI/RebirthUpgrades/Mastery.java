...
public class Mastery extends AbstractItem {
    private UUID playerUUID;

    public Mastery(UUID playerUUID) {
        this.playerUUID = playerUUID;
    }
    @Override
    public ItemProvider getItemProvider() {
        if ((boolean) accessPlayerData(playerUUID, "Mastery")) {
        return new ItemBuilder(Material.GREEN_DYE).setDisplayName("§a+ Mastery §8| §c15♦")
                .addLoreLines("§7Your clicker gives §a+200")
                .addLoreLines("§7and §a3x§7 the §a+1§7, the §a+200")
                .addLoreLines("§7is applied before any of")
                .addLoreLines("§7the other multipliers")
    ...
    public void handleClick(@NotNull ClickType clickType, @NotNull Player player, @NotNull InventoryClickEvent inventoryClickEvent) {
        ...
        if (clickType.isLeftClick()) {
            if (RebirthPoints >= Cost) {
                ...
                storePlayerData(player.getUniqueId(), "Mastery", true);
