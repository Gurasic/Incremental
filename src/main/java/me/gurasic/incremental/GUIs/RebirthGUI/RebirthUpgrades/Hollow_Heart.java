...
public class Hollow_Heart extends AbstractItem {
    private UUID playerUUID;

    public Hollow_Heart(UUID playerUUID) {
        this.playerUUID = playerUUID;
    }
    @Override
    public ItemProvider getItemProvider() {
        if ((boolean) accessPlayerData(playerUUID, "Hollow_Heart")) {
        return new ItemBuilder(Material.GREEN_DYE).setDisplayName("§aHollow Heart §8| §c25♦")
                .addLoreLines("§7Your Maximum health is")
                .addLoreLines("§7Increased by §c2❤")
    ...
    public void handleClick(@NotNull ClickType clickType, @NotNull Player player, @NotNull InventoryClickEvent inventoryClickEvent) {
        ...
        if (clickType.isLeftClick()) {
            if (RebirthPoints >= Cost) {
                ...
                storePlayerData(player.getUniqueId(), "Hollow_Heart", true);
